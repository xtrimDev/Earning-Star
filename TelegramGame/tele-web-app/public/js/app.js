    ...
    return axios.get(`${protocol}//${host}/get/${what}`)
        ...
        .catch(function (error) {
            Swal.fire({
                ...
                position: 'top-end',
                icon: 'warning',
                title: "Something went wrong, Refresh the page and check again",
...
document.getElementsByClassName('link')[0].addEventListener('click', async () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(`${await get("TeleBotUrl")}?startapp=${await get("myReferral")}`).then(function () {
        ...
        textarea.value = `${await get("TeleBotUrl")}?startapp=${await get("myReferral")}`;
...
document.getElementsByClassName('fb-share')[0].addEventListener('click', async () => {
    ...
    let shareUrl = `${protocol}//${host}/auth/register?startapp=${await get("myReferral")}`;
...
document.getElementById("run-spin").addEventListener("click", async () => {
    ...
    async function runSequences() {
        ...
        const withdrawalAmt = parseInt(await get("withdrawalAmount"));

        $("#totalEarningsToWithdrwal").text(function (i, oldText) {
            earn = (parseInt(totalEarnings) + y);
            if (earn < withdrawalAmt) {
                return withdrawalAmt - earn;
            } else {
                Swal.fire({
                    toast: true,
                    position: 'top-end',
        ...
        let xyz = (((parseInt(totalEarnings) + y) * 100)/withdrawalAmt);

        document.getElementById("earningProgress").style.width = xyz + "%";
...
$("#withdrawForm").submit(async function (form) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            } else if (response.trim() == "disabled") {
                loader.style.display = "none";

                Swal.fire({
                    title: 'Disabled',
                    text: "Transaction are disabled by the owner, pls try again later.",
                    icon: 'info',
                })
            }
...
$("#treasure-hunt").click(async() => {
    loader.style.display = "flex";
    const selector = document.querySelector("#tresure-hunt-hist .main-dialog ol");

    if (selector) {
        selector.remove()
    }

    const result = await get("TaskChannelList");
    ...
    if (result && result != "failed" && result.length > 0 ) {
        ...
        for (const channel of result)  {
            ...
            li.innerHTML = `<a href="https://t.me/${channel.username}/" style="text-decoration: none; color: White; font-size: 1.3rem; text-transform: capitalize">@${channel.username}</a>`;
            ol.append(li);
        }

        const selector = document.querySelector("#tresure-hunt-hist .main-dialog .caption-treasure-hunt");
        selector.after(ol);

    }

    $("#tresure-hunt-hist").css("display", "flex")

    $("#tresure-hunt-hist-close").click( () => {
        $("#tresure-hunt-hist").css("display", "none");
    })
